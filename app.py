...
def validate_and_extract(src_df):
    ...
    # ❗️Diferența esențială: citim din df_norm (nu din src_df)
    out = pd.DataFrame({
        "Warehouse": df_norm[col_wh],
        "Material Code": df_norm[col_mat].astype(str),
        "Quantity": pd.to_numeric(df_norm[col_qty], errors="coerce"),
        "New Price": pd.to_numeric(df_norm[col_price], errors="coerce"),
    })
...
if uploaded:
    try:
        ...
        # 🔎 Afișare diagnostic: capete de coloană normalizate
        st.caption("Capete de coloană detectate (normalizate):")
        st.code(", ".join(list(normalize_columns(src_df).columns)))
